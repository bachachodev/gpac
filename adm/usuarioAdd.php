...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "agregar")) {
  $insertSQL = sprintf("INSERT INTO `access` (persNumeroDocumento, acceUsr, accePwd, acceNivel, acceDescripcion) VALUES (%s, %s, %s, %d, %s)",
...
if (substr($_SERVER['QUERY_STRING'], 0, 6) == "numDoc") { //Link
  ...
  $colname_rsEdit = $numDoc;
}
mysql_select_db($database_rsPP, $rsPP);
$query_rsEdit = sprintf("SELECT * FROM access WHERE persNumeroDocumento = %s", GetSQLValueString($colname_rsEdit, "text"));
...
if ($_SERVER['QUERY_STRING'] == "new") : ?>
  <div Id="contAlert" class="alert alert-warning fade in" align="center" role="alert">
      ...
      <span class="sr-only">Error</span>Registro inexistente. Agregar usuario
  </div> 
<?php endif; ?>
...
if (isset($requsername)) : ?>
  <div Id="contAlert" class="alert alert-danger fade in" align="center" role="alert">
    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    <span class="glyphicon glyphicon-remove" aria-hidden="true" style="font-size:22px; vertical-align:middle"></span>
    <span class="sr-only">OK</span>Nombre de usuario <strong>< <?php echo $requsername; ?> ></strong> ya existe
  </div>
<?php endif; ?>
    ...
    <div class="panel-body">
	  ...
	  <?php //***** Si existe usuario creado
		if ($totalRows_rsEdit > 0) {
		  echo "Esta persona ya tiene un usuario creado";
		}
		else {
		  echo "Nuevo usuario";
		}
      ...
      <!--tab content-->
      <?php if ($totalRows_rsEdit == 0) : //**** Si no existe usuario creado ?>
      <div class="tab-content">
        <div id="datos" class="tab-pane fade in active">
          <div class="form-group input-group" style="width:100%;">
            <input type="hidden" name="numeroDocumento" id="numeroDocumento" value="<?php echo $row_rsPersUser['persNumeroDocumento']; ?>" required>
          </div>
          <div class="form-group input-group" style="width:100%;">
            <span class="input-group-addon" style="width:90px"><span class="glyphicon glyphicon-user" style="color:orange"></span></span>
            <input type="text" name="user" id="user" value="" class="form-control" placeholder="Usuario" autofocus onfocus="this.value=''" autocomplete="new-user" required/>
          </div>
          <div class="form-group input-group" style="width:100%">
            <span class="input-group-addon" style="width:90px"><span class="glyphicon glyphicon-align-left" style="color:orange"></span></span>
            <input type="password" name="pwd" id="pwd" value="" class="form-control" placeholder="Contraseña" autofocus onfocus="this.value=''" autocomplete="new-password" required/>
          </div>
