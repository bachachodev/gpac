...
$maxRows_rsEdit2 = 1;
$pageNum_rsEdit2 = 0;
if (isset($_GET['pageNum_rsEdit2'])) {
  $pageNum_rsEdit2 = $_GET['pageNum_rsEdit2'];
}
$startRow_rsEdit2 = $pageNum_rsEdit2 * $maxRows_rsEdit2;
...
mysql_select_db($database_rsPP, $rsPP);
$query_rsEdit2 = sprintf("SELECT * FROM persona WHERE persNumeroDocumento = %s", GetSQLValueString($colname_rsEdit2, "text"));
$query_limit_rsEdit2 = sprintf("%s LIMIT %d, %d", $query_rsEdit2, $startRow_rsEdit2, $maxRows_rsEdit2);
$rsEdit2 = mysql_query($query_limit_rsEdit2, $rsPP) or die(mysql_error());
$row_rsEdit2 = mysql_fetch_assoc($rsEdit2);

if (isset($_GET['totalRows_rsEdit2'])) {
  $totalRows_rsEdit2 = $_GET['totalRows_rsEdit2'];
} else {
  $all_rsEdit2 = mysql_query($query_rsEdit2);
  $totalRows_rsEdit2 = mysql_num_rows($all_rsEdit2);
}
$totalPages_rsEdit2 = ceil($totalRows_rsEdit2/$maxRows_rsEdit2)-1;
...
//if ((empty($_POST['numeroDocumento'])) || (empty($_POST['paterno']))) {
if (($case_rsedit = 1) || ($case_rsedit = 2)) {
  $query_rsEdit = sprintf("SELECT * FROM persona WHERE $var_rsedit Like %s ORDER BY persPaterno ASC", GetSQLValueString($colname_rsEdit . $like_rsedit, "text"));
}
else {
  $query_rsEdit = sprintf("SELECT * FROM persona WHERE $var_rsedit = %s ORDER BY persPaterno ASC", GetSQLValueString($colname_rsEdit . $like_rsedit, "text"));
}
...
<?php 
if (isset($_POST['find'])) { echo "| FIND: Set ";} else { echo "| FIND: Null ";}
if (isset($_POST['numeroDocumento'])) { echo "| SET NUM: Set ";} else { echo "| SET NUM: Null ";}
if (empty($_POST['numeroDocumento'])) { echo "| NUM: vacio ";} else { echo "| NUM:". $_POST['numeroDocumento'] ." ";}
if (isset($_POST['paterno'])) { echo "| SET PATERNO: Set ";} else { echo "| SET PATERNO: Null ";}
if (empty($_POST['paterno'])) { echo "| PATERNO: vacio ";} else { echo "| PATERNO:". $_POST['paterno'] ." ";}
echo "| COLNAME: ".$colname_rsEdit." ";
echo "| VAR: ".$var_rsedit." ";
echo "| LIKE: ".$like_rsedit." ";
echo "| TOTAL: ".$totalRows_rsEdit." ";
echo $row_rsEdit['persNumeroDocumento'];
echo "| CASE: ".$case_rsedit." ";
?>

<form method="POST" action="personaFind.php?<?php if ($_SERVER['QUERY_STRING'] == "edit") : ?>edit<?php endif; ?><?php if ($_SERVER['QUERY_STRING'] == "del") : ?>del<?php endif; ?>" class="form-signin" name="buscar" id="buscar" role="form">
  <!--panel-->
  <div class="panel panel-default">
    ...
    <div class="panel-footer">
      ...
      <input type="hidden" name="find" value="<?php if ($_SERVER['QUERY_STRING'] == "edit") : ?>editar<?php endif; ?><?php if ($_SERVER['QUERY_STRING'] == "del") : ?>eliminar<?php endif; ?>">
    </div>
  </div>
</form>
      ...
      <table class="table table-hover">
        <thead>
          <tr>
            ...
            <th><?php do { ?>
                Número Documento
                <?php } while ($row_rsEdit2 = mysql_fetch_assoc($rsEdit2)); ?></th>
            <th>Paterno</th>
            <th>Nombres</th>
            <th>Nacionalidad</th>
          </tr>
