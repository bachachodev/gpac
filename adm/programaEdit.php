...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "editar")) {
  $updateSQL = sprintf("UPDATE programa SET progNombre=%s, progAreaAcademica=%s, progVigencia=%s WHERE progID=%s",
...
$colname_rsEdit = "-1";
if (isset($_GET['progCodigo'])) {
  $colname_rsEdit = $_GET['progCodigo'];
}
mysql_select_db($database_rsPP, $rsPP);
$query_rsEdit = sprintf("SELECT * FROM programa WHERE progCodigo = %s", GetSQLValueString($colname_rsEdit, "text"));
      ...
      <div class="panel-body">
        ...
        <!--tab content-->
            ...
            <span class="input-group-addon" style="width:90px"><span class="glyphicon glyphicon-barcode" style="color:orange"></span></span>
              <input type="text" name="codigo" id="codigo" value="<?php echo $row_rsEdit['progCodigo']; ?>" class="form-control" placeholder="Código Programa" autofocus autocomplete="off" disabled>
            </div>
            <div class="form-group input-group" style="width:100%;">
              <span class="input-group-addon" style="width:90px"><span class="glyphicon glyphicon-book"></span></span>
              <input type="text" name="nomPrograma" id="nomPrograma" value="<?php echo $row_rsEdit['progNombre']; ?>" class="form-control" placeholder="Nombre Programa" autofocus autocomplete="off">
            </div>
            <div class="form-group input-group" style="width:100%">
              <span class="input-group-addon" style="width:90px"><span class="glyphicon glyphicon-education"></span></span>
              <input type="text" name="areaAcademica" id="areaAcademica" value="<?php echo $row_rsEdit['progAreaAcademica']; ?>" class="form-control" placeholder="Area Académica" autofocus autocomplete="off">
            </div>
            <div class="checkbox checkbox-warning">
            <input name="vigencia" type="checkbox" class="styled" id="vigencia" value="1" <?php if (!(strcmp($row_rsEdit['progVigencia'],1))) {echo "checked=\"checked\"";} ?>>
            <label for="usuario">Vigente</label>
            </div>
          </div>
          <!--footer-->
        <div class="panel-footer">
        <input type="hidden" name="progID" value="<?php echo $row_rsEdit['progID']; ?>">
